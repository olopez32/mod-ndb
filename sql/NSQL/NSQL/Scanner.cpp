...
void Scanner::Init() {
	...
	keywords.set(L"from", 4);
	keywords.set(L"where", 5);
	keywords.set(L"order", 6);
	keywords.set(L"or", 7);
	keywords.set(L"and", 8);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 2:
			...
			else if (ch <= L'!' || (ch >= L'#' && ch <= L'[') || (ch >= L']' && ch <= 65535)) {AddCh(); goto case_2;}
			else if (ch == 92) {AddCh(); goto case_3;}
			else {t->kind = noSym; break;}
		case 3:
			case_3:
			if (ch == L'"' || ch == 92) {AddCh(); goto case_2;}
			else {t->kind = noSym; break;}
		case 4:
			case_4:
			{t->kind = 9; break;}
